...
In Section~\ref{sec:spec} we present the high-resolution spectroscopy from MMT/Hectochelle, which we used to define a sample of highly probable GD-1 members (\S\ref{sec:membership}).
...
After implementing the radial velocity constraint, the allowed locations of the GD-1's perturber are still not entirely localized on the sky, but instead distributed approximately along a great circle.
...
% Despite the poor localization, current constraints on the GD-1's perturber are already informative of its origin.
...
The Sagittarius dwarf is itself too massive to produce the spur-and-gap morphology, but a compact, low-mass object associated with Sagittarius, like a globular cluster or a dark-matter subhalo, is a plausible culprit.
- if the perturber is associated w sgr, encounter rates need to be reevaluated (before assumed isotropic)
- the structural properties derived similar to gc, but none of the known gcs comes closer than 1kpc in the past 2 Gyr, even with the updated orbit
- should search to test this hypothesis, possible the census incomplete
% (in particular for sgr, expected emosaics, associated massari)
- if luminous object not found, a dark matter subhalo still possible, although more in line w sidm?

% There are uncertainties associated both with modeling of the Sagittarius debris (e.g., simulated proper motions disagree with the observations) and inference on the GD-1's perturber (e.g., the parameter space of the encounter might be more complex than currently mapped) which make their association tentative at the present, and there remain sufficient support in the distribution of plausible GD-1 perturbers for one unassociated with Sagittarius.
% - more generally, if encounter, and associated w sgr, encounter rates need to be reevaluated (before assumed isotropic)

% - mention sgr supposed to have an impact from de boer (+ alternative spur geometry)
% - perhaps can explain other gd-1 features
- other explanations of the complex morphology (de boer, kshir, progenitor there)
- here are the data that need to be satisfied
- if predictions we can refer to, do
...
- new opportunity for directly predicting where the perturber is now
